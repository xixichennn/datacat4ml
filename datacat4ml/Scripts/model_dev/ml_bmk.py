...
from datacat4ml.Scripts.const import Algos, Input_dirs, DESCRIPTORS, rmvSs, Aim_Spl_combinations
...
from datacat4ml.Scripts.const import FEAT_HHD_OR_DIR, FEAT_MHD_effect_OR_DIR, FEAT_MHD_OR_DIR, FEAT_LHD_OR_DIR
from datacat4ml.Scripts.const import ML_HP_DIR, ML_DIR
    ...
    if not os.path.isfile(bmk_file):
        with open(bmk_file, 'w') as f:
            f.write('algo,'
                    ...
                    'descriptor,SPL,aim_spl_combo,rmvS,'
                    'ds_size,ds_size_level,threshold,percent_a,int_outer_n_fold,'
                    'pipeline,'
                    'auroc,auprc,balanced,kappa,bedroc,f1,mcc,accuracy\n'
            )
    count = 0
        ...
        if all(required):
            ...
            with open(bmk_file, 'a') as f:
                f.write(f'{algo.__name__},'
                        ...
                        f'{descriptor},int,{data_info["aim_spl_combo"]},rmvS{rmvS},'
                        f'{data_info["ds_size"]},{data_info["ds_size_level"]},{data_info["threshold"]},{data_info["percent_a"]},{data_info["int_outer_n_fold"]},'
                        f'{pl},'
                        f'{metrics["auroc"]},{metrics["auprc"]},{metrics["balanced"]},{metrics["kappa"]},{metrics["bedroc"]},{metrics["f1"]},{metrics["mcc"]},{metrics["accuracy"]}\n'
                        )
        else:
    ...
    if not os.path.isfile(bmk_file):
        with open(bmk_file, 'w') as f:
            f.write('algo,'
                    ...
                    'size_diff,'
                    'pipeline,'
                    'pf_auroc,pf_auprc,pf_balanced,pf_kappa,pf_bedroc,pf_f1,pf_mcc,pf_accuracy,'
                    'cf_auroc,cf_auprc,cf_balanced,cf_kappa,cf_bedroc,cf_f1,cf_mcc,cf_accuracy\n'
            )
        ...
        for pf_prefix, cf_prefixes in pfp_cfps_map.items():
            ...
            if len(cf_prefixes) == 0:  
                ...
                for cf_prefix in cf_prefixes:
                    ...
                    size_diff = abs(pf_data_info['ds_size'] - cf_data_info['ds_size'])
                    ...
                    if all(required):
                        ...
                        with open(bmk_file, 'a') as f:
                            f.write(f'{algo.__name__},'# algo: e.g. RF
                                    ...
                                    f'{descriptor},aln,{pf_data_info["aim_spl_combo"]},rmvS{rmvS},' # pf_data_info and cf_data_info have the same aim and spl
                                    ...
                                    f'{size_diff},'
                                    f'{pl},'
                                    f'{pf_metrics["auroc"]},{pf_metrics["auprc"]},{pf_metrics["balanced"]},{pf_metrics["kappa"]},{pf_metrics["bedroc"]},{pf_metrics["f1"]},{pf_metrics["mcc"]},{pf_metrics["accuracy"]},'
                                    f'{cf_metrics["auroc"]},{cf_metrics["auprc"]},{cf_metrics["balanced"]},{cf_metrics["kappa"]},{cf_metrics["bedroc"]},{cf_metrics["f1"]},{cf_metrics["mcc"]},{cf_metrics["accuracy"]}\n'
                                    )
                    else:
...
if __name__ == '__main__':
    ...
    if args.bmk_type == 'int':
        bmk_fname = f'bmkML_int_{args.algo}_{args.descriptor}_{args.aim_spl_combo.split("_")[0]}_rmvS{args.rmvS}_{args.aim_spl_combo.split("_")[1]}_{args.pl}_{args.input_dir.split("/")[-1]}.csv'
        ...
        bmk_fname = f'bmkML_aln_{args.algo}_{args.descriptor}_{args.aim_spl_combo.split("_")[0]}_rmvS{args.rmvS}_{args.aim_spl_combo.split("_")[1]}_{args.pl}.csv'
        bmk_file=os.path.join(ML_DIR, bmk_fname)
