...
class MLData:
    ...
    def __init__(self, file_folder: str, filename: str, task: str, use_smote: bool = False):
        ...
        self.smiles_train = df[df['split'] == 'train']['CanonicalSMILES'].tolist() #Yu after split, use 'CanonicalSMILES'
        if self.task == 'reg':
            self.y_train = df[df['split'] == 'train']['pStandard_value'].tolist()
        elif self.task == 'cls':
            self.y_train = df[df['split'] == 'train']['activity'].tolist()
        #self.cliff_mols_train = df[df['split'] == 'train']['cliff_mol'].tolist() # Yu check if it is necessary

        # The testing dataset
        self.smiles_test = df[df['split'] == 'test']['CanonicalSMILES'].tolist() #Yu
        if self.task == 'reg':
            self.y_test = df[df['split'] == 'test']['pStandard_value'].tolist()
        elif self.task == 'cls':
            self.y_test = df[df['split'] == 'test']['activity'].tolist()
        #self.cliff_mols_test = df[df['split'] == 'test']['cliff_mol'].tolist() #Yu
    ...
    def shuffle(self):
        ...
        """
        #Yu: remove the code related with 'cliff_mols_train' if not used later.
        if self.task == 'reg':
    ...
    # Yu: remove it if later not used.
