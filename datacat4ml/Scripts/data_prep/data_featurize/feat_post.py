...
# merge_feat_pkl
#=============================================================================
def merge_feat_pkls(in_dir, rmv_dupMol: int = 0):
    """
    ...
    # output directory
    pkl_dir = os.path.join(Spl_Feat_Dic[in_dir], f'rmvDupMol{str(rmv_dupMol)}')
...
# get_feat_stats
#=============================================================================
def get_feat_stats(in_dir: str = FEAT_HHD_OR_DIR, ds_cat_level: str = 'hhd', ds_type: str = 'or', rmv_dupMol: int = 0):
    """
    Get the statistics of the featurized datasets and save them to a csv file.
    """

    print(f'Processing: {in_dir}')
    feat_path = os.path.join(in_dir, f'rmvDupMol{str(rmv_dupMol)}')
    ...
    for f in feat_files:
        ...
        ################################ write the stats ################################
        stats_file_path = os.path.join(FEAT_DATA_DIR, f'feat_{ds_cat_level}_{ds_type}_rmvDupMol{rmv_dupMol}_stats.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rmv_dupMol', type=int, help='Indicate the location of the input directory.', default=0)
    ...
    for in_dir in keys_list:
        print(f'Processing {in_dir}...')
        merge_feat_pkls(in_dir, rmv_dupMol=args.rmv_dupMol)

    #============ get_feat_stats =================
    get_feat_stats(in_dir=FEAT_HHD_OR_DIR, ds_cat_level='hhd', ds_type='or', rmv_dupMol=args.rmv_dupMol)
    get_feat_stats(in_dir=FEAT_MHD_OR_DIR, ds_cat_level='mhd', ds_type='or', rmv_dupMol=args.rmv_dupMol)
    get_feat_stats(in_dir=FEAT_LHD_OR_DIR, ds_cat_level='lhd', ds_type='or', rmv_dupMol=args.rmv_dupMol)
    get_feat_stats(in_dir=FEAT_MHD_effect_OR_DIR, ds_cat_level='mhd_effect', ds_type='or', rmv_dupMol=args.rmv_dupMol)