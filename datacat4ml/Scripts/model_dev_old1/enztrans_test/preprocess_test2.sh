...
dataset_dir=${workdir}/data/ki_mor_1_test2

# if the output directory does exist, delete it
if [ -d ${dataset_dir}/preprocessed_data ]; then
    rm -r ${dataset_dir}/preprocessed_data
fi

if [ -d ${dataset_dir}/model ]; then
    rm -r ${dataset_dir}/model
fi

if [ -d ${dataset_dir}/Tensorboard ]; then
    rm -r ${dataset_dir}/Tensorboard
fi
...
# remove two lines below from the above script
